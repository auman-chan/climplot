...
#'
#' @return A Walter & Lieth climatic diagram of the provided locations,
...
clim_plot <- function(data,
  ...
  if (ymin < -1.5) {
    ...
    if (ymin < -10) {
      for (i in (ymin / 10 + 1):-1) {
        t_lab <- c(t_lab, i * 10) # minimum temperature value
        p_lab <- c(p_lab, "")
      }
    }
    t_lab <- c(t_lab, "0", "10", "20", "30", "40", "50", "")
    p_lab <- c(p_lab, "0", "20", "40", "60", "80", "100", "300")
  } else {
    t_lab <- c("0", "10", "20", "30", "40", "50", "")
    p_lab <- c("0", "20", "40", "60", "80", "100", "300")
  }
  if (ymax > 60) {
    for (i in 6:(ymax / 10 - 1)) {
      t_lab <- c(t_lab, "")
      p_lab <- c(p_lab, 100 * (2 * i - 7))
    }
  }
  plot(0:13 - 0.5, c(tm[12], tm[1:12], tm[1]),
    xlim = c(0, 12),
  ...
  axis(2, ((lmin / 10):(ymax / 10)) * 10, labels = t_lab, col.axis = tcol)
  axis(4, ((lmin / 10):(ymax / 10)) * 10, labels = p_lab, col.axis = pcol)
  ...
  mtext(paste(round(mean(tm * 10)) / 10, "\U{00B0}C"),
    line = 2, adj = 1, cex = 1.2
  )
  mtext(paste(round(sum(p)), " mm", sep = ""),
    line = 1, adj = 1, cex = 1.2
  )
    ...
    for (i in 2:14) {
      if (gr) {
        ...
        if (p2[i] > 100) {
          ...
          # 2. the month with prec less than 100mm
          # but the last month more than 100mm
          xp[n] <- x[i - 1] + (100 - p2[i - 1]) / (p2[i] - p2[i - 1])
          # x coordinate of turning point of
          # wet season polygon would have scale-change
          yp[n] <- 50
          # y coordinate of turning point of
          # wet season polygon,set as position 50(C).
